...
import EnemyCamp from './camps.mjs'
...
class World{
    ...
    this.campChar='\u2B22';
      ...
      warpQuestStage: 0,
      gotFirstWanderer: false
		}, loadedGame.world);
    this.camps = $.extend([], EnemyCamp.from(loadedGame.camps));
    ...
    if(this.wanderer.exists && this.world.gotFirstWanderer){
    ...
    if(this.camps.length < 4 && Math.random() < 0.5){
      let camp = new EnemyCamp(this).dieInAbout(45, 60);
    }
    render(this);
	}
	spaceAction(x,y){
        ...
        this.world.gotFirstWanderer = true;
			}
		}
    for(let i in this.camps){
      debugger;
      let camp = this.camps[i];
      if(camp.pos[0] == this.player.pos[0] && camp.pos[1] == this.player.pos[1]){
				camp.raid();
			}
    }
        ...
        music.soundtracks.expansion.start().next('safe');
			}
  ...
  spawnWanderer(nocycle){
    ...
    if(nocycle) return;
		...
		if(this.world.gotFirstWanderer) this.wandererKillTimer = setTimeout(window.world.killWanderer.bind(window.world), Math.floor(Math.random()*5000+20000));
	}
	killWanderer(){
		this.wanderer.exists=false;
		render(this);
