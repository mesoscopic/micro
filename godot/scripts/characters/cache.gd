...
func _hurt() -> void:
	...
	weights.add_item("funds", 20)
	if Micro.player.hp < Micro.player.max_hp: weights.add_item("heal", 12)
	if global_position.length_squared() > 900: weights.add_item("surprise", 3)
