...
func serialize_input(event: InputEvent) -> String:
	if event is InputEventKey:
		...
		return "axis:%s" % ((event.axis + 1)*int(sign(event.axis_value)))
	return ""
...
func load_scenes() -> void:
	...
	scenes[&"micro:player_damage_cloud"] = load("uid://q68wlnumgicy")
	...
	scenes[&"micro:bullet_beam"] = load("uid://cjga5r42rtiuw")
